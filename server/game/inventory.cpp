...
std::optional<uint8_t> Inventory::find_free_slot() const {
    for (uint8_t i = 0; i < capacity; ++i) {
...
void Inventory::recount_occupied() {
...
const InventorySlot& Inventory::at(uint8_t index) const {
...
bool Inventory::place(ItemType type, const std::string& name) {
    std::optional<uint8_t> free_idx = find_free_slot();
    if (!free_idx)
        return false;
...
void Inventory::load_slots(const std::vector<InventorySlot>& new_slots) {
    ...
    for (uint8_t i = 0; i < capacity; ++i) {
        if (i < new_slots.size()) {
            ...
            slots.push_back(InventorySlot{i, ItemType::NONE, ""});
        }
    }
    recount_occupied();
}
...
void Inventory::from_records(const std::vector<InventorySlotRecord>& records) {
    std::vector<InventorySlot> converted;
    converted.reserve(records.size());
    for (const auto& rec: records) {
        InventorySlot slot;
        slot.item_type = static_cast<ItemType>(rec.item_type);
        slot.item_name = std::string(rec.item_name, std::strlen(rec.item_name));
        converted.push_back(std::move(slot));
    }
    load_slots(converted);
}

bool Inventory::has_free_slot() const { return occupied < capacity; }

uint8_t Inventory::first_free_slot() const { return find_free_slot().value_or(capacity); }
