...
uint8_t PlayerInventory::slot_count() const { return equip_capacity + hp_capacity + mana_capacity; }

bool PlayerInventory::is_empty_at(uint8_t index) const { return inv_for(index).is_empty_at(index); }

const InventorySlot& PlayerInventory::at(uint8_t index) const { return inv_for(index).at(index); }

void PlayerInventory::clear(uint8_t index) { inv_for(index).clear(index); }
