...
RegenService::RegenService(std::map<uint16_t, Player>& players, const BalanceConfig& balance,
...
bool RegenService::apply_regen_channel(double& accum, double rate, uint32_t cur, uint32_t max,
...
CommandResult RegenService::apply_regen() {
    ...
    for (auto& [id, player]: players_) {
        ...
        changed |= apply_regen_channel(hp_regen_accum_[id], hp_rate, player.get_hp_current(),
                                       player.get_hp_max(),
        ...
        changed |= apply_regen_channel(mana_regen_accum_[id], mana_rate, player.get_mana_current(),
                                       player.get_mana_max(),
