...
bool FileManager::save_as(TilemapDocument& doc) {
    QString path = QFileDialog::getSaveFileName(parent_, "Save Map As",
        QString::fromStdString(doc.path().empty()
            ? "config/common_tilemap.toml" : doc.path()),
        "TOML files (*.toml)");
...
bool FileManager::open(TilemapDocument& doc) {
    QString path = QFileDialog::getOpenFileName(parent_, "Open Map",
        QString::fromStdString(doc.path().empty()
            ? "config" : doc.path()),
        "TOML files (*.toml)");
    if (path.isEmpty()) return false;
