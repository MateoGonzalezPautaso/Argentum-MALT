...
ClientCommand wrap(std::string arg) {
    return Cmd{std::move(arg)};
}
}  // namespace
...
std::optional<ClientCommand> ChatCommandParser::parse(const std::string& text) const {
    ...
    if (text.starts_with("/depositar oro ")) {
        try {
            return BankDepositCmd{true, "",
                                  static_cast<uint32_t>(std::stoul(arg("/depositar oro ")))};
        } catch (...) {
            return std::nullopt;
        }
    }
    if (text.starts_with("/retirar oro ")) {
        try {
            return BankWithdrawCmd{true, "",
                                   static_cast<uint32_t>(std::stoul(arg("/retirar oro ")))};
        } catch (...) {
            return std::nullopt;
        }
    }
    if (text.starts_with("/depositar "))
        return BankDepositCmd{false, arg("/depositar "), 0};
    if (text.starts_with("/retirar "))
        return BankWithdrawCmd{false, arg("/retirar "), 0};
    if (text.starts_with("/equipar ")) {
        try {
            return EquipItemCmd{static_cast<uint8_t>(std::stoi(arg("/equipar ")))};
        } catch (...) {
            return std::nullopt;
        }
    }
    if (text.starts_with("/desequipar ")) {
        ...
        if (slot == "weapon")
            return UnequipItemCmd{EquipSlot::WEAPON};
        if (slot == "armor")
            return UnequipItemCmd{EquipSlot::ARMOR};
        if (slot == "helmet")
            return UnequipItemCmd{EquipSlot::HELMET};
        if (slot == "shield")
            return UnequipItemCmd{EquipSlot::SHIELD};
        return std::nullopt;
    }
