...
void SpriteRenderer::despawn_entity(uint16_t entity_id) { entities_.erase(entity_id); }

void SpriteRenderer::clear_all_entities() { entities_.clear(); }
...
void SpriteRenderer::render(const SDL2pp::Rect& cam) {
    ...
    for (auto& [entity_id, e]: entities_) {
        ...
        if (body_it == e.parts.end() || (!body_it->use_src && !is_npc))
            continue;
        const SpriteRender& movable_entity = *body_it;
        if (!is_visible(movable_entity, cam))
            continue;

        append_equip_overlay_drawables(movable_entity, e.equip.overlays, e.equip.static_cache, cam,
                                       drawables);
    }
